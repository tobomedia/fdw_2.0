...
casper.start().thenOpen("https://www.spotlight.com/agentweb/Security/Login", function() {
...
casper.then(function() {
    ...
    casper.evaluate(function(username,password) {
        document.getElementById("usernameTextBox").value='clare';
		document.getElementById("passwordTextBox").value='v1ncent';
...
casper.thenOpen("https://www.spotlight.com/agentweb/Clients/PinList/all/csv", function() {
    ...
    for (var i = 0; i < parsed.length; i++) {
        casper.thenOpen("https://www.spotlight.com/"+parsed[i].pin, function() {
            var name = this.evaluate(function() {
                ...
                if (n) {
                    return n.innerText.toLowerCase().replace(/\s+/g,'_').replace(/\'/g,'');
                } else {
                    return null;
                }
            });
            ...
            if (name) {
                ...
                name = this.evaluate(function() {
                    return document.querySelector('.artistName').innerText.toLowerCase().replace(/\s+/g,'_').replace(/\'/g,'');
                });
            }
